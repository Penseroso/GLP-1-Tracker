...
| Official pipeline stage marker versus trial-initiated stage/status | protocol clarification candidate before v1 freeze | Zealand petrelintide is described with planned Phase 3 initiation, and petrelintide / CT-388 has a Phase 2 registry entry that is not yet recruiting; Ascletis ASC35 has IND clearance but no started Phase 1 trial. | Store the stage supported by initiated or active development evidence; store regulatory milestones separately. | clarify protocol | The contract supports this, but v1 guidance should explicitly say promotional or forward-looking pipeline markers do not advance stage without initiation evidence. |
